...
public class BoardAuthenticationFilter implements Filter{
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain)
		 ...
		 * 필터 - 게시글 수정, 삭제에 대한 필터(내가 작성한 글만 수정, 삭제가 가능함)
			현재 세션에 값 == 작성자가 동일한 경우 삭제 or 수정 가능
			
			1.글쓰기시에 작성자는 사실 고정값 (세션값을 이용해서)
			2. 수정 삭제 요청이 들어올떄는, 작성자에 대한 정보(이메일)를 반드시 넘기고 == 현재 세션과 비교
		 */
		...
		//작성자가 없거나, 작성자가 세션값이 일치하지 않는 경우에 목록으로 이동
